...
class TestMapExprReduceFusion(unittest.TestCase):
    def setUp(self):
        self.inputs = {
            "x": np.random.uniform(-1.0, 1.0, [2, 1024]).astype("float32"),
            "y": np.random.uniform(-1.0, 1.0, [2, 1024]).astype("float32"),
        }
    ...
    def test_reduce_fusion(self):
        ...
        np_expect = self.inputs["x"] + self.inputs["y"]
        np_expect = np.maximum(np_expect, 0)
        np_expect = np_expect + self.inputs["y"]
        np_expect = np.sum(np_expect, axis=0)
