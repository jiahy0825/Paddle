...
struct IsLhsBeforeRhsStruct<std::int64_t, std::int64_t> {
  static bool Call(std::int64_t lhs, std::int64_t rhs) {
    return lhs < rhs;
  }
};
...
struct IsLhsBeforeRhsStruct<Reciprocal<DimExpr>, Reciprocal<DimExpr>> {
  static bool Call(const Reciprocal<DimExpr>& lhs, const Reciprocal<DimExpr>& rhs) {
    const auto& [lhs_operand] = lhs.tuple();
    const auto& [rhs_operand] = rhs.tuple();
    return IsLhsBeforeRhs(lhs_operand, rhs_operand);
  }
...
template<>
struct IsLhsBeforeRhsStruct<BroadcastedDim<DimExpr>, BroadcastedDim<DimExpr>> final :
    public IsListLhsBeforeListRhsStruct<BroadcastedDim> {
};

bool IsLhsBeforeRhs(const DimExpr& lhs, const DimExpr& rhs) {
  return std::visit([&](const auto& lhs, const auto& rhs){
    return IsLhsBeforeRhsStruct<std::decay_t<decltype(lhs)>, std::decay_t<decltype(rhs)>>::Call(lhs, rhs);
...
struct SortOperands {
  ...
  DimExpr Rewrite(const DimExpr& expr) {
    ...
    List<DimExpr> mut_operands{};
    mut_operands->insert(mut_operands->end(), operands->begin(), operands->end());
...
struct VisitEachInversableOperandStruct {
  ...
  static void Call(const DimExpr& expr, const DoEachT& DoEach, std::size_t depth, bool is_inversed) {
...
template<>
struct VisitEachOperandStruct<Sum> : public VisitEachInversableOperandStruct<Sum, Negative> {};

template<>
struct VisitEachOperandStruct<Product> : public VisitEachInversableOperandStruct<Product, Reciprocal> {};
...
void VisitEachOperand(const DimExpr& expr, const DoEachT& DoEach) {
  if (expr.Has<Op<DimExpr>>()) {
    VisitEachOperandStruct<Op>::Call(expr, DoEach, /*depth=*/0, /*is_inversed=*/false);
  } else {
    // Do nothing;
  }
}
...
bool HasNested(const auto& expr) {
  ...
  VisitEachOperand<Op>(expr, [&](const DimExpr& operand, std::size_t depth, bool is_negative){
...
struct GetInversed<Product> {
  static DimExpr Call(const DimExpr& expr) {
    return Reciprocal<DimExpr>(expr);
  }
};
...
struct FlattenOperands {
  ...
  DimExpr Rewrite(const DimExpr& expr) {
    ...
    VisitEachOperand<Op>(expr, [&](const DimExpr& dim_expr, std::size_t depth, bool is_negative){
      if (is_negative) {
...
struct FoldConstants {
  ...
  DimExpr Rewrite(const DimExpr& expr) {
    ...
    List<DimExpr> ret_operands{};
    typename FoldOperandTrait<Op>::const_value_type const_dim = FoldOperandTrait<Op>::MakeUnit();
    for (const auto& operand : *operands) {
    ...
    if (ret_operands->empty()) {
      FoldOperandTrait<Op>::MakeDimExpr(FoldOperandTrait<Op>::MakeUnit(), &ret_operands);
    }
...
struct FoldInversedPairToUnit {
  ...
  std::optional<SearchResult> SearchInversedPair(const List<DimExpr>& operands) {
    for (int i = 0; i < operands->size(); ++i) {
      for (int j = 0; j < operands->size(); ++j) {
        if (i == j) {
          continue;
        }
        if (FoldOperandTrait<Op>::IsInversedPair(operands->at(i), operands->at(j))) {
