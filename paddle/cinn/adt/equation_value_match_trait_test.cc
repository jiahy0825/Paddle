...
TEST(Match, Union) {
  using Pattern = Union<IndexUnDotValue<Value, List<DimExpr>>, IndexDotValue<Value, List<DimExpr>>>;
  {
    Value expr =
        IndexUnDotValue<Value, List<DimExpr>>{Value{Ok()}, List<DimExpr>{std::int64_t(1)}};
  ...
  {
    Value expr =
        IndexDotValue<Value, List<DimExpr>>{Value{Ok()}, List<DimExpr>{std::int64_t(1)}};
...
TEST(Match, list_get_item_index_undot) {
  Value undot1 =
      IndexUnDotValue<Value, List<DimExpr>>{Value{Ok()}, List<DimExpr>{std::int64_t(1)}};
  ASSERT_TRUE((cinn::adt::Match<IndexUnDotValue<Value, List<DimExpr>>>(undot1)));

  Value expr = ListGetItem<Value, DimExpr>{undot1, DimExpr{std::int64_t(1)}};
  ASSERT_TRUE(
      (cinn::adt::Match<
          ListGetItem<IndexUnDotValue<Value, List<DimExpr>>, std::int64_t>>(expr)));
}
...
TEST(Match, list_list_get_item_index_undot) {
  ...
  ASSERT_TRUE(
      (cinn::adt::Match<
          ListGetItem<IndexUnDotValue<Value, List<DimExpr>>, std::int64_t>>(expr1)));
  Value expr2 = ListGetItem<Value, DimExpr>{undot, DimExpr{std::int64_t(1)}};
  ASSERT_TRUE(
      (cinn::adt::Match<
          ListGetItem<IndexUnDotValue<Value, List<DimExpr>>, std::int64_t>>(expr2)));
...
TEST(Match, index_dot_list_list_get_item_index_undot) {
  ...
  ASSERT_TRUE(
      (cinn::adt::Match<
          List<ListGetItem<IndexUnDotValue<Value, List<DimExpr>>, std::int64_t>>>(
          list)));
  Value dot = IndexDotValue<Value, List<DimExpr>>{list, List<DimExpr>{std::int64_t(1)}};
  ASSERT_TRUE(
