...
struct ToTxtStringStruct {
  ...
  std::string operator()(const SymbolicDimExpr& value) {
    std::string ret;
    ret += ToTxtString(value);
    return ret;
  }
  ...
  std::string operator()(const ListGetItem<Value, SymbolicDimExpr>& list_get_item) {
    std::string ret;
  ...
  std::string operator()(
      const BroadcastedIterator<Value, SymbolicDimExpr>& broadcast) {
    std::string ret;
