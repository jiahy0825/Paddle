...
class KGroup final {
 public:
  explicit KGroup(const std::shared_ptr<dialect::ir::Group>& cinn_group,
                  const std::vector<std::shared_ptr<IGroup>>& igroups)
      : cinn_group_(cinn_group), igroups_(igroups) {}

  std::shared_ptr<dialect::ir::Group> cinn_group() const {
 ...
 private:
  std::weak_ptr<dialect::ir::Group> cinn_group_;
