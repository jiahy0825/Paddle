...
DEFINE_ADT_UNION(Value,
                 ...
                 SymbolicDimExpr,
                 List<Value>,
                 IndexDotValue<Value, List<SymbolicDimExpr>>,
                 IndexUnDotValue<Value, List<SymbolicDimExpr>>,
                 ListGetItem<Value, SymbolicDimExpr>,
                 BroadcastedIterator<Value, SymbolicDimExpr>,
                 PtrGetItem<Value>);

OVERLOAD_OPERATOR_EQ_NE(Value, UnionEqual);
using IndexDot_Value_List_SymbolicDimExpr = IndexDotValue<Value, List<SymbolicDimExpr>>;
OVERLOAD_OPERATOR_EQ_NE(IndexDot_Value_List_SymbolicDimExpr, TupleEqual);
using IndexUnDot_Value_List_SymbolicDimExpr = IndexUnDotValue<Value, List<SymbolicDimExpr>>;
OVERLOAD_OPERATOR_EQ_NE(IndexUnDot_Value_List_SymbolicDimExpr, TupleEqual);
using ListGetItem_Value_SymbolicDimExpr = ListGetItem<Value, SymbolicDimExpr>;
OVERLOAD_OPERATOR_EQ_NE(ListGetItem_Value_SymbolicDimExpr, TupleEqual);
using BroadcastedIterator_Value_SymbolicDimExpr = BroadcastedIterator<Value, SymbolicDimExpr>;
OVERLOAD_OPERATOR_EQ_NE(BroadcastedIterator_Value_SymbolicDimExpr, TupleEqual);
...
inline std::size_t GetHashValueImpl(const SymbolicDimExpr& value) {
...
inline std::size_t GetHashValueImpl(
    const IndexDotValue<Value, List<SymbolicDimExpr>>& value) {
...
inline std::size_t GetHashValueImpl(
    const IndexUnDotValue<Value, List<SymbolicDimExpr>>& value) {
  const auto& [v, c] = value.tuple();
  std::size_t hash_value = GetHashValue(v);
  for (const auto& expr : *c) {
    hash_value = hash_combine(hash_value, GetHashValue(expr));
...
inline std::size_t GetHashValueImpl(const ListGetItem<Value, SymbolicDimExpr>& value) {
  const auto& [v, c] = value.tuple();
  return hash_combine(GetHashValue(v), GetHashValue(c));
}
inline std::size_t GetHashValueImpl(
    const BroadcastedIterator<Value, SymbolicDimExpr>& value) {
  const auto& [v, c] = value.tuple();
  return hash_combine(GetHashValue(v), GetHashValue(c));
}
