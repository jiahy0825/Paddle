...
struct GroupTileInfo {
  ...
  // group->loop_ranges.size()
  int64_t data_rank;

  // flatten_inner_num * warp_num * 32 == flatten_block
  // flatten_numel / flatten_block
  int64_t block_num{-1};
  // flatten_block / 128 or reduce_block / 256 or reduce_numel / warp_size
  int64_t warp_num;
  // constant = flatten_block / (warp_num * 32)
